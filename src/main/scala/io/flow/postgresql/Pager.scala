...
object Pager {
  ...
  /** Helper to create a pager with an offset, inferring the types from the function returning each page of results.
    ...
    * Example:
    *
    * Pager.create { offset => SubscriptionsDao.findAll( publication = Some(Publication.DailySummary), offset = offset )
    * }.foreach { subscription => println("subscription: " + subscription) }
    */
  ...
  /** Helper to create a pager using on the last seen id. This pager is useful when scanning large tables where using
    * the offset may incur a performance hit
    *
    * Example:
    *
    * Pager.byLastId[T, Id]( { lastId: Option[Id] => SubscriptionsDao.findAllWhereIdGreaterThan( publication =
    * Some(Publication.DailySummary), minId = lastId ) }, subscription: T => subscription.id ).foreach { subscription =>
...
trait GenericPager[T, Context] extends Iterator[T] {
  ...
  /** Updates the context keeping track of the traversal of the elements based on the next element produced. This
    * function is called every time the iterator produces the next element.
    * @param element
    *   the next element produced
    */
  def updateContext(currentContext: Context, element: T): Context

  /** Produces the next page of elements
    * @param context
    *   the context keeping track of the traversal the elements
    * @return
    *   the next page of elements
    */
  ...
  /** Produces the next element of this iterator.
    * @return
    *   the next element of this iterator, if `hasNext` is `true`, throws a [[scala.NoSuchElementException]] otherwise.
    */
  override def next(): T = {
...
trait LastIdPager[T, Id] extends GenericPager[T, Option[Id]] {
  ...
  /** Extracts the id of the element. This function is called when the next element is produced to extract its id
    */
